...
`/debts`: сопоставление строк таблицы с **любым** учеником в БД по фамилии и имени (без отчества). Выбор предмета в `/subjects` не обязателен для попадания в отчёт; если предмет не выбран — в тексте будет пометка.
